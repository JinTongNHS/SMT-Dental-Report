################################################################################
plot_banded_CoT_averge_2019 <- function(data = UDA_scheduled_data, 
  ...
  data_to_print <- get_banded_COTs_data(data, historic_data = historic_data, remove_prototypes = remove_prototypes, 
  ...
  if(asIndex == TRUE){
    ...
    data2019 <- data %>%
      ...
      group_by(band) %>% 
      summarise(total_2019 = sum(CoTs)) %>% 
      mutate (CoTs_2019 = total_2019/12) %>% 
      select(##month_number,
             band,
             CoTs_2019) ## = CoTs)
      
      data <- data %>%
        left_join(data2019, by = c("band")) %>%
