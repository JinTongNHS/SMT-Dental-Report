################################################################################
plot_UDA_UOA_delivery_with_contracted_values <- function(data = UDA_scheduled_data, 
  ...
  if(UDAorUOA == "UDA"){
    ...
    data <- get_delivery_data_with_contracted(data, remove_prototypes, UDAorUOA = "UDA", all_regions_and_STPs = all_regions_and_STPs)
    title <- "Monthly UDA delivery and monthly* contracted UDAs"
    ylab <- "Number of UDAs delivered or contracted"
    captionTitle <- "*Monthly contracted UOAs is calculated by dividing annual contracted UOAs by 12 (or 18 for April)
                     *Excluding contracts with annual contracted UDA < 100. Excluding prototype contracts up until April 2022."
    legendLabels <- c("Monthly UDAs delivered", "Monthly UDAs contracted")
    ...
    #get data into the right format
    data <- get_delivery_data_with_contracted(data, remove_prototypes, UDAorUOA = "UOA", all_regions_and_STPs = all_regions_and_STPs)
    title <- "Monthly UOA delivery and monthly* contracted UOAs"
    ylab <- "Number of UOAs delivered or contracted"
    captionTitle <- "*Monthly contracted UOAs is calculated by dividing annual contracted UOAs by 12 (or 18 for April)
                    **Excluding contracts with no annual contracted UOAs. Excluding prototype contracts up until April 2022."
    legendLabels <- c("Monthly UOAs delivered", "Monthly UOAs contracted")
  ...
  if(plotChart == TRUE){
    ...
    data <- data %>%
      ungroup() %>%
      select(month, monthly_UDA_UOAs_delivered, scaled_monthly_UDA_UOAs_contracted) %>%
      pivot_longer(cols = !month, names_to = "contracted_or_delivered", values_to = "UDA_UOAs")
    ...
    p <- ggplot(data = data) +
      ...
      geom_line(aes(x = month,
                    y = UDA_UOAs,
                    colour = contracted_or_delivered),
                size = 1) +
    ...
    p <- p +
      ...
      scale_colour_manual(values = c("deepskyblue3", "coral"), labels = legendLabels) +
      labs(title = title,
           x = "Month",
           y = ylab,
           subtitle = subtitle,
