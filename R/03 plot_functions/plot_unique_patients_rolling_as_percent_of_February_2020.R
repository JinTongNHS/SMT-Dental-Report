...
plot_unique_patients_rolling_as_percent_of_feb_20 <- function(data = unique_patients_rolling,
  ...
  if(asIndex == TRUE){
    ...
    feb2020 <- data %>%
      ...
      select(-month,
             feb2020_total_unique_patients = total_unique_patients)
    
    data <- data %>%
      left_join(feb2020, by = "band") %>%
      mutate(total_unique_patients = total_unique_patients * 100 / feb2020_total_unique_patients)
    
    title <- "Total number of unique patients seen over a rolling 12 month period by band \nas a percentage of February 2020 figures"
